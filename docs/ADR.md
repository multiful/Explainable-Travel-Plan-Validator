<!-- updated: 2026-09-16 | hash: 2427fbaf | summary: Neo4j 제거와 배포 의존성 manifest 기준을 기록한 ADR -->
...
**이유**: 당시 커버리지 검증 결과, Neo4j가 담당하던 7개 역할을 Python 로직이 100% 대체함을 확인.
...
### ADR-008: 의존성 관리 — pyproject.toml + requirements.txt
**결정**: `pyproject.toml`을 프로젝트 선언의 기준으로 삼고, `requirements.txt`를 Docker·CI 배포용 미러로 유지한다. 개발 설치는 `pip install -e ".[dev]"`를 사용한다.
**이유**: 표준 Python 패키지 구조. `src/` 레이아웃으로 import 충돌을 방지하고, 배포 환경은 별도 빌드 메타데이터 없이 requirements 파일을 설치한다. 두 manifest의 런타임 패키지 집합은 테스트로 동기화한다.
