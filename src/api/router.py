...
from src.scoring.congestion_engine import CongestionEngine
...
_ROUTE_MATRIX = RouteMatrixService.from_settings()
_CONGESTION_ENGINE = CongestionEngine.from_settings()
...
def _build_jeju_place_index() -> dict[str, dict]:
    ...
    for values in rows:
        ...
        hrs = _parse_jeju_hours(str(row.get("TourAPI_영업시간") or "")) or _parse_jeju_hours(
            str(row.get("영업시간") or "")
        )
        if hrs:
    ...
    if bf_path.exists():
        ...
        for item in bf_list:
            ...
            try:
                ...
                has_c = False
            bf_tags = ["무장애"]
            if item.get("wheelchair"):
                bf_tags.append("휠체어")
            if item.get("elevator"):
                bf_tags.append("엘리베이터")
            if item.get("stroller"):
                bf_tags.append("유모차")
            if item.get("parking"):
                bf_tags.append("주차가능")
            result.append(
...
_FULL_PLACE_LIST += [
    ...
    for k, e in _JEJU_CSV_INDEX.items()
    if k not in _seen_full
]
            ...
            filtered = filtered + [
                {
                    ...
                    "cat": code,
                    "cat_name": (
                        kp.category_name.split(">")[-1].strip() if kp.category_name else "장소"
                    ),
                    "has_coords": True,
                    "annual_max": 0.0,
                    "firstimage": "",
...
async def list_regions() -> dict:
    ...
    for p in _FULL_PLACE_LIST:
        ...
        if r:
            counts[r] += 1
    return {
        "regions": [{"name": k, "count": v} for k, v in sorted(counts.items(), key=lambda x: -x[1])]
    }
...
async def parse_document(file: UploadFile) -> ParsedPlanResponse:
...
async def validate_plan(req: ValidateRequest) -> ValidateResponse:
    ...
    unresolved = [
        name
        for (_, name, _), (_poi, info) in zip(flat_places, resolved, strict=True)
        if not info.found
    ]
    if unresolved:
        raise HTTPException(
    ...
    plan = ItineraryPlan(
        days=[DayPlan(places=[PlaceInput(name=p.name) for p in day.places]) for day in req.days],
    ...
    all_pois = [poi for day in per_day_pois for poi in day]
    await _ROUTE_MATRIX.prepare(all_pois)
    ...
    for info in poi_info_list:
        live_congestion = _CONGESTION_ENGINE.score(info.name, travel_month)
        if live_congestion.fallback_used == "seoul_realtime":
            if live_congestion.congestion_score >= 0.65:
                congestion_warnings.append(
                    {
                        "poi_name": info.name,
                        "month": travel_month,
                        "level": live_congestion.level.value.lower(),
                        "ratio": round(live_congestion.congestion_score, 2),
                        "message": (
                            f"서울 실시간 혼잡도는 {live_congestion.level.value} 수준이에요. "
                            "가능하면 이른 방문을 추천해요."
                        ),
                        "best_month": None,
            ...
            congestion_warnings.append(
                {
                    ...
                    "message": (
                        ...
                        f"예약이나 이른 방문을 추천해요."
                        if level == "high"
                        else f"{MONTH_NAMES[travel_month]}은 사람이 꽤 많은 편이에요. "
                        f"여유 있게 시간을 잡으세요."
                    ),
